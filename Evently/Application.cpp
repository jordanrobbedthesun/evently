...
namespace EventlyGUI {
    ...
    void RenderCalendarView() {
        ...
        if(ImGui::Button("##invisiblegamerkatbutton", ImVec2(20, 10))){}
            ...
            for (size_t week = 0; week < 6; ++week)
            {
                ...
                for (size_t day = 0; day < 7; ++day) {
                    ...
                    if (currentMonth.getDay(week, day) != 0) {
                        ...
                        const float hues[6] = {0.0f, 0.08f, 0.16f, 0.33f, 0.56f, 0.76f};
                        for (int i = 0; i < events.size(); i++){
                            ...
                            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, (ImVec4)ImColor::HSV(hue, 0.7f, 0.6f));
                            ImGui::PushStyleColor(ImGuiCol_ButtonActive, (ImVec4)ImColor::HSV(hue, 0.8f, 0.7f));
    ...
    void CreateEventWindow() {
        if (showCreateEventWindow) {
            ...
            ImGui::Combo("##tag", &selectedColor, colors, IM_ARRAYSIZE(colors));
            ...
            if (ImGui::Button("Create")) {
                Event event(selectedYear, selectedMonth + 1, selectedDay, selectedHour, selectedMinute, selectedDuration, selectedTimeZone, eventName, eventLocation, eventDescription, selectedColor);
    ...
    void EventInfo(){
        if(showEventInfoWindow){
            ...
            ImGui::Text("Color: "); ImGui::SameLine();
            ImGui::Text(event.getColorString().c_str());
